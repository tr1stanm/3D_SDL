...
void renderPoints(double angle) {
	...
	for(int i = 0; i < 20000; ++i) {
		...
		toDraw = rotateX(rotateY(rotateZ(toDraw, angle * 1.1), angle * 1.12), angle);
...
gsl_matrix* createPoint(gsl_matrix *point) {
	...
	if(r < .25) point = f1_3D(point);
	else if(r < .5) point = f2_3D(point);
	else if(r < .75) point = f3_3D(point);
	else point = f4_3D(point);
